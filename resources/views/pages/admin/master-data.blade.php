...
@section('content')
    <div class="p-6">
        ...
        <div class="flex gap-1 mb-5 bg-gray-100 dark:bg-gray-800 rounded-xl p-1 w-fit">
            ...
            <button onclick="switchTab('tpl')" id="tabTpl"
                class="px-4 py-2 rounded-lg text-sm font-medium transition-all text-gray-500 hover:text-gray-700 dark:text-gray-400">Template
                Excel</button>
        </div>
        ...
        <div class="mb-4 flex gap-3 items-center">
            <div class="relative w-80">
                ...
                <input id="searchInput" type="text" placeholder="Cari..."
                    class="w-full pl-9 pr-4 py-2 text-sm bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-800 dark:text-white"
                    oninput="doSearch()">
            </div>
        ...
        <!-- Item Pane -->
        <div id="paneItem" style="display:none" class="w-full">
            <div
                class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden shadow-sm">
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left border-collapse">
        ...
        <!-- Template Excel Pane -->
        <div id="paneTpl" style="display:none" class="w-full">
            <div
                class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden shadow-sm">
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left border-collapse">
    ...
    <div id="modalCat" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60"
        ...
        <div
            ...
            <div class="p-6 space-y-4">
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1.5">Nama Kategori <span
        ...
        <div
            ...
            <div class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1.5">Kode Item <span
                                class="text-red-500">*</span></label>
                        <input id="itemCode" type="text" placeholder="ITM-001"
                            class="w-full px-4 py-2.5 text-sm bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-800 dark:text-white">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1.5">Satuan <span
                                class="text-red-500">*</span></label>
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1.5">Nama Item <span
                            class="text-red-500">*</span></label>
                    <input id="itemName" type="text" placeholder="Nama item"
                        class="w-full px-4 py-2.5 text-sm bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-800 dark:text-white">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1.5">Kategori <span
                            class="text-red-500">*</span></label>
    ...
    <div id="modalTpl" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60"
        style="display:none!important">
        <div
            class="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-md mx-4 shadow-2xl border border-gray-200 dark:border-gray-700">
            <div class="flex justify-between items-center px-6 py-4 border-b border-gray-100 dark:border-gray-800">
            ...
            <div class="p-6 space-y-4">
                <div id="alertTpl" class="hidden p-3 rounded-xl text-sm bg-red-50 text-red-600 border border-red-100">
                </div>
                <input type="hidden" id="tplId">
                <div>
                    <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1.5">Judul Template <span
                            class="text-red-500">*</span></label>
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1.5">File Excel <span
                            id="tplFileRequired" class="text-red-500">*</span></label>
            ...
            <div class="px-6 pb-6 flex gap-3">
                <button onclick="saveTpl()"
                    class="flex-1 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-medium">Simpan</button>
                <button onclick="hideModal('modalTpl')"
                    class="flex-1 py-2.5 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 rounded-xl text-sm">Batal</button>
            </div>
        </div>
    </div>
    ...
    <div id="modalDel" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60"
        ...
        <div
            ...
            <div class="flex gap-3">
                ...
                <button onclick="hideModal('modalDel')"
    ...
    <script>
        ...
        function openEditCat(id) {
            const c = allCats.find(x => x.id_master_category == id);
            if (!c) return;
        ...
        async function saveCat() {
            ...
            if (!name) {
                setAlert('alertCat', 'Nama wajib diisi.');
                return;
            }
            ...
            try {
                ...
                if (!res.ok) {
                    setAlert('alertCat', json.message || errMsg(json.errors));
                    return;
                }
                hideModal('modalCat');
                ...
                setAlert('alertCat', 'Terjadi kesalahan.');
            }
        }
        ...
        function openEditItem(id) {
            const item = allItems.find(x => x.id_item == id);
            if (!item) return;
        ...
        async function saveItem() {
            ...
            const body = {
                ...
                id_category: document.getElementById('itemCatSel').value,
                description: document.getElementById('itemDesc').value.trim()
            };
            if (!body.item_code || !body.item_name || !body.unit || !body.id_category) {
                setAlert('alertItem', 'Semua field wajib diisi.');
                return;
            }
            ...
            try {
                ...
                const json = await res.json();
                if (!res.ok) {
                    setAlert('alertItem', json.message || errMsg(json.errors));
                    return;
                }
                hideModal('modalItem');
                ...
                setAlert('alertItem', 'Terjadi kesalahan.');
            }
        }
        ...
        function populateCatSelects() {
            const opts = '<option value="">-- Pilih Kategori --</option>' + allCats.map(c =>
                `<option value="${c.id_master_category}">${c.name}</option>`).join('');
            document.getElementById('itemCatSel').innerHTML = opts;
            document.getElementById('catItemFilter').innerHTML = '<option value="">Semua Kategori</option>' + allCats.map(
                c => `<option value="${c.id_master_category}">${c.name}</option>`).join('');
        }
        ...
        async function saveTpl() {
            ...
            try {
                ...
                const json = await res.json();
                if (!res.ok) {
                    setAlert('alertTpl', json.message || errMsg(json.errors));
                    return;
                }
                hideModal('modalTpl');
                toast(id ? 'Template diperbarui' : 'Template diupload');
                await loadTpls();
            } catch (e) {
                setAlert('alertTpl', 'Terjadi kesalahan.');
            }
        }
        async function loadTpls() {
            const res = await fetch('/api/admin/rfq-templates', {
                headers: {
                    'Accept': 'application/json'
                }
            });
        ...
        async function confirmDel() {
            const urlMap = {
                cat: `/api/admin/master-categories/${delId}`,
                item: `/api/admin/master-items/${delId}`,
                tpl: `/api/admin/rfq-templates/${delId}`
            };
            const res = await fetch(urlMap[delType], {
                method: 'DELETE',
                headers: {
                    'Accept': 'application/json',
                    'X-CSRF-TOKEN': CSRF()
            ...
            if (res.ok) {
                ...
                if (delType === 'cat') await loadCats();
                else if (delType === 'item') await loadItems();
                else await loadTpls();
            } else {
