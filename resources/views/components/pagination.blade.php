<div id="{{ $id }}" class="pagination-root flex items-center justify-between px-5 py-3.5 border-t border-gray-100 dark:border-gray-800">
...
window.Pagination = (() => {
    ...
    function _render(id) {
        ...
        const { total, perPage, currentPage, callback } = inst;
    ...
    function _bindEvents(id) {
        ...
        root.querySelector('.pagination-prev').addEventListener('click',  () => _goTo(id, _instances[id].currentPage - 1));
        root.querySelector('.pagination-next').addEventListener('click',  () => _goTo(id, _instances[id].currentPage + 1));
    ...
    return {
        init(id, total, perPage = 10, callback = () => {}) {
            _instances[id] = { total, perPage, currentPage: 1, callback };
            _bindEvents(id);
            _render(id);
        },
        ...
        currentPage(id)    { return _instances[id]?.currentPage ?? 1; },
        currentPerPage(id) { return _instances[id]?.perPage ?? 10; },
    };
})();
</script>
@endpush
